...
uint32_t CProcess_Manager::Sbrk(uint32_t size){
    ...
    if(task->heap_start == 0){
        task->heap_start = 0x20000;
        task->logical_break = 0x20000;
        task->physical_break = 0x20000;
    }

    //aktualni proces, v ramci semestralky to muze byt jenom ten nas model task
    ...
    while(address >= task->physical_break){
        uint32_t* pt = reinterpret_cast<uint32_t*>(task->cpu_context.ttbr0);
        //alokuj novou stranku
        uint32_t heap_page_phys = static_cast<unsigned long>(sPage_Manager.Alloc_Page()) - mem::MemoryVirtualBase;
        //posun na konec nove alokovane stranky
        task->physical_break += mem::PageSize;
        //namapuj virtualni pamet
        map_memory(pt, heap_page_phys, task->physical_break);
    }
    //vrat zacatek volneho mista

    return address;

    /*
    uint16_t code_phys_start = 0x00000000;
    //zacatek kodove sekce - za kodovou sekci chceme placnout haldu
    //zaber tolik stranek, kolik chce a placni je kontinualne za sebe
    for(int i = 1; i <= size; i++){
        uint32_t heap_page_phys = static_cast<unsigned long>(sPage_Manager.Alloc_Page()) - mem::MemoryVirtualBase;
        map_memory(pt, heap_page_phys, code_phys_start + mem::PageSize * i);
    }
    ...
    return code_phys_start + mem::PageSize;
    */
     /*
    //if(size == 5)
      //  sUART0.Write('x');
    //zaber stranky, ktere odpovidaji @param size mega
    // Process_Heap_Chunk_Header* chunk = reinterpret_cast<Process_Heap_Chunk_Header*>(sPage_Manager.Alloc_Page());
    Process_Heap_Chunk_Header* start = sProc_Heap_Manager.Alloc_Heap_Space(size);

    //zacatek pameti, virtualni pamet bude naplacana za sebou
    uint8_t* mem_start = reinterpret_cast<uint8_t*>(start) + sizeof(Process_Heap_Chunk_Header);
    int i = 1;

    while(start){

        uint32_t heap_page_phys = (static_cast<unsigned long>(reinterpret_cast<uint32_t>(start)) + sizeof(Process_Heap_Chunk_Header)) - mem::MemoryVirtualBase;
        map_memory(pt, heap_page_phys, code_phys_start + mem::PageSize * i);
        i++;
        start = start->next;
    }

    //update pt po pridani adres s haldou
...
uint32_t CProcess_Manager::Create_Process(unsigned char* elf_file_data, unsigned int elf_file_length, bool is_system)
{
    ...
    task->logical_break = 0; //relativni break, na zacatku nemam haldu, ukazuju primo za code section
    task->physical_break = 0;// absolutni break, na zacatku nemam haldu
    task->heap_start = 0;   //zacatek haldy
    ...
    // alokujeme stranku pro kod, zasobnik a data
    ...
    //uint32_t data_page_phys = static_cast<unsigned long>(sPage_Manager.Alloc_Page()) - mem::MemoryVirtualBase;
...
//spusti prvni task a neresi nic dalsiho
void CProcess_Manager::RunFirstTask(){
    CProcess_List_Node* node = mProcess_List_Head;
    if(!node) return;
...
void CProcess_Manager::Handle_Process_SWI(NSWI_Process_Service svc_idx, uint32_t r0, uint32_t r1, uint32_t r2, TSWI_Result& target)
{
    ...
    {
        //TODO if time
        case NSWI_Process_Service::Free:
            target.r0 = 6;
            sUART0.Write('\n');
            break;
        case NSWI_Process_Service::Malloc:
            //r0 - kolik byteu chce proces na halde alokovat
            target.r0 = Sbrk(r0);
            break;
